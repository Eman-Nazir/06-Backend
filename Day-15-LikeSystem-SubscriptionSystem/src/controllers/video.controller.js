...
// GET ALL VIDEOS — search, filter, sort, paginate
...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  // 1. SEARCH — regex search on title and description
  if (query) {
    pipeline.push({
      $match: {
        $or: [
  ...
  if (minDuration || maxDuration) {
    ...
    if (minDuration) durationFilter.$gte = Number(minDuration);
    if (maxDuration) durationFilter.$lte = Number(maxDuration);
    pipeline.push({
  ...
  if (minViews) {
    pipeline.push({
      $match: { views: { $gte: Number(minViews) } },
    });
  }

  // 5. ONLY PUBLISHED VIDEOS
